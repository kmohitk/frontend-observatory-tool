...
export type ObservatoryOverlay = {
  setLcp: (v: string) => void;
  setCls: (v: string) => void;
  setLongTask: (v: string) => void;
};

export function createObservatoryOverlay(): ObservatoryOverlay {
...
export type StartCollectionOptions = {
  onMetric: (metric: Metric) => void;
  /** If set, updates floating HUD when values change */
  overlay?: ObservatoryOverlay | null;
};

export function startPerformanceCollection(options: StartCollectionOptions): void {
  const { onMetric, overlay } = options;
